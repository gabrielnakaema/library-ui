...
def initialize_db():
    ...
    insertOneWithdrawalQuery = QSqlQuery(connection)
    insertOneWithdrawalQuery.exec(
        """
        INSERT INTO withdrawals (book_barcode, student_name, student_grade, withdrawal_date, is_returned, return_date) values ('1809a28b-a9b4-49e8-b8b0-beef6d280a00', 'abc', '2', '29/10/2020', False, '1/1/1970')
        """
    )
    selectAllBooksQuery = QSqlQuery(connection)
    selectAllBooksQuery.exec("SELECT * FROM books")
    if selectAllBooksQuery.size() < 5:
        insert_mock_data()
...
def insert_mock_data():
    ...
    '''
    queryObject = QSqlQuery(connection)
    for query in queries.split(';'):
        if len(query.strip()) > 0:
            queryObject.exec(query + ';')
